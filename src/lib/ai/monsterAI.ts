...
//
// This is the **default** AI used when an entity has no custom `ai` state machine.
// The entity `ai: { initial, states, transitions }` config object is the primary
// extension point; the framework's turn dispatcher calls the active state function
// each tick instead of this module.

import { aStar8 } from "./astar";
import type { GridPos, AStar8Options } from "./astar";
import type { DungeonOutputs } from "../dungeon/bsp";
import type { TurnAction, ActorId, MonsterActor, MonsterAlertState } from "../turn/types";
...
function _pathTo(
  ...
  const opts: AStar8Options = {};
  if (fourDir !== undefined) opts.fourDir = fourDir;
  const result = aStar8(dungeon, isWalkable, { x: sx, y: sy }, { x: gx, y: gy }, opts);
